...
active_img.value = props.item.image

function handleActiveImg(image) {
...
<template>
  <section class="product__details-area pb-115">
    ...
    <div class="container">
      <div class="row">
        <div v-if="item" class="col-xl-7 col-lg-6">
              ...
              <nav>
                <div class="nav nav-tabs justify-content-sm-between">
                  ...
                  <button v-for="(item, i) in item?.gallery" :key="i"
                    :class="`nav-link ${item.image === active_img ? 'active' : ''}`"
                    @click="handleActiveImg(item.image)">
                    <!-- {{ item.image }} -->
          ...
          <div class="product__details-wrapper">
            ...
            <div class="product__details-rating d-flex align-items-center">
              ...
              <div class="product__details-rating-count">
                <span>({{ item?.review_count }} customer review)</span>
              </div>
            </div>
            ...
            <div class="product__details-action d-flex flex-wrap align-items-center">
              <!-- Add To Cart Button -->
              <button  @click="cart.addItem(item)" class="product-add-cart-btn product-add-cart-btn-3">
            ...
            <div class="product__details-sku product__details-more">
              <p>SKU:</p> <span>{{ item?.sku }}</span>
            </div>
            <div class="product__details-categories product__details-more">
              <p class="">Categories:</p>
              <span>
                <RouterLink to="#">{{ item?.category }}</RouterLink>
              </span>
            </div>
